			...
			submitHandler: function(ev)
			{
				...
				setTimeout(function()
				{
					...
					$.ajax({
						...
						dataType: 'json',
						data: {
							user_code: $("#login_user_code").val().toUpperCase(),
						...
						error: function()
						{
							...
							alert("ID / PW 를 확인해 주십시요.");
							$("#login_error_text").text("ID / PW 를 확인해 주십시요.").trigger("change");
							return false;
						},
						success: function(response)
						{
							...
							setTimeout(function()
							{
								if(response.result == "NG")
								{
									...
									$("#login_error_text").text(response.message).trigger("change");
									return false;
								}
								else{
									setTimeout(function()
			...
			resetProgressBar: function(display_errors)
			{
				...
				setTimeout(function()
				{
					...
					if(display_errors)
					{
						...
						neonLogin.$container.find('input[type="user_pwd"]').val('');
					}
